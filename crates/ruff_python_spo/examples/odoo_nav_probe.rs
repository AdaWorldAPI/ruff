...
fn main() {
    let root = std::env::args().nth(1).expect("usage: odoo_nav_probe <addon_root>");
    // Vocab: the account-addon core screens (normalized model names).
    let screens = [
        "account_move", "account_move_line", "res_partner", "res_company",
        "account_account", "account_journal", "account_payment",
        "account_bank_statement", "account_analytic_line", "account_tax",
    ];
    ...
    println!(
        ...
        report.py_files, report.xml_files, report.raw_act_window_refs,
        report.files_with_edges, edges.len()
    );
    for e in &edges {
